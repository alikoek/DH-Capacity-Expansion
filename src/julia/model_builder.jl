...
function build_sddp_model(params::ModelParameters, data::ProcessedData)
    ...
    model = SDDP.MarkovianPolicyGraph(
        ...
        @variables(sp, begin
            0 <= X_store[tech in params.technologies] <= 3000,  SDDP.State, initial_value == params.c_initial_capacity[tech]
            0 <= X_tech[tech in params.technologies] <= 3000,   SDDP.State, initial_value == params.c_initial_capacity[tech]
        ...
        if t % 2 == 1
            ...
            @variables(sp, begin
                0 <= U_techn[tech in params.technologies] <= params.c_max_additional_capacity[tech]
                0 <= U_store <= params.storage_params[:max_capacity]
            end)
        ...
        ####### Operational Stage #######
            ...
            @variables(sp, begin
                0 <= u_production[tech in params.technologies, week=1:data.n_weeks, hour=1:data.hours_per_week]
